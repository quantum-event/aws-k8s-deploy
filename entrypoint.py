...
def check_aws_credentials():
    ...
    if not (access_key and secret_key) and not role_to_assume:
        print("Either AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY must be set, or AWS_ROLE_TO_ASSUME must be defined. Exiting...")
        sys.exit(1)

    if role_to_assume:
        print(f"AWS Role to assume: {role_to_assume}")
    else:
        print(f"Using AWS_ACCESS_KEY_ID: {access_key} and AWS_SECRET_ACCESS_KEY.")

def validate_arguments(namespace, deployments):
    if not isinstance(namespace, str) or not namespace.strip():
        print(f"Invalid or missing namespace. It must be a non-empty string.")
        sys.exit(1)
    ...
    for deploy in deployments:
        for field in required_fields:
            ...
            if not isinstance(value, str) or not value.strip():
                print(f"Field '{field}' is missing or not a valid non-empty string in deployment: {deploy}")
                sys.exit(1)
...
def run_kubectl_set_image(namespace, deployment, container, image):
    try:
        ...
        print(f"Deployment {deployment} successfully updated in namespace {namespace}.")
...
def run_kubectl_rollout_status(namespace, deployment):
    try:
        ...
        subprocess.run(command, check=True)
        print(f"Rollout status for deployment {deployment} successfully checked.")
    except subprocess.CalledProcessError as e:
